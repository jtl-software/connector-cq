...
namespace Jtl\Connector\Cq\Sniffs\Commenting;

use Jtl\Connector\Cq\Types;
        ...
        if ($return !== null) {
            ...
            if (empty($content) === true || $tokens[($return + 2)]['code'] !== T_DOC_COMMENT_STRING) {
                ...
                foreach ($typeNames as $typeName) {
                    $suggestedName = Types::suggestType($typeName);
    ...
    protected function checkInheritdoc(File $phpcsFile, $stackPtr, $commentStart): bool
    {
        ...
        for ($i = $commentStart; $i <= $tokens[$commentStart]['comment_closer']; $i++) {
            if (in_array($tokens[$i]['code'], $allowedTokens) === false) {
                ...
                if ($trimmedContent === '@inheritdoc' || $trimmedContent === '{@inheritdoc}') {
                    return true;
                } else {
                    return false;
                }
